"use client";

interface EndPaperProps {
  side: 'left' | 'right';
}

/**
 * Non-editable end paper component
 * Displays as a dark panel with subtle messaging
 */
export default function EndPaper({ side }: EndPaperProps) {
  return (
    <div className="w-full h-full bg-slate-800 flex items-center justify-center">
      <div className="text-center text-slate-400">
        <svg
          className="w-16 h-16 mx-auto mb-3 opacity-30"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={1}
            d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"
          />
        </svg>
        <p className="text-sm font-medium uppercase tracking-wide opacity-60">
          This page cannot be edited
        </p>
        <p className="text-xs mt-1 opacity-40">
          This end paper cannot be edited.
        </p>
      </div>
    </div>
  );
}
